		...
		getData: function() {
			...
			self.calendar = Calendar.create('#myCalendar',{
				...
				onSelect: function(o){
					...
					self.dateTime = t;
					var url = C.Config.getRewardDataListUrl() + '?callback=C.JCLQ.initReward&lotteryTypeId=' + C.Config.lotteryTypeName + '&dateTime=' + t + '&t=' + new Date().getTime();
					Tool.getScript(url);
				},
			...
			//get Data
			var url = C.Config.getRewardDataListUrl() + '?callback=C.JCLQ.initReward&lotteryTypeId=' + C.Config.lotteryTypeName + '&dateTime=' + self.dateTime + '&t=' + new Date().getTime();
			Tool.getScript(url);
		},
